...
- **Row order.** `pilot_many_events` and `pilot_head_to_head` came back with the same rows in a different order. They are the `ORDER_INSENSITIVE` queries, so `compare` tolerates it by design, but the captured JSON is written in row order and therefore differs on every capture.
- **Float drift.** `gems_whole_meta` and `gems_one_archetype` returned identical rows under `_identity` with `mean_norm` moved in the last bits. Max observed delta **1.11e-16**, against the **5.6e-17** the `TOLERANCE` comment cites as the largest measured difference. That figure was measured engine-to-engine (Kùzu vs Ladybug); this is Ladybug against itself across two runs of the same binary, so the documented number understates what the harness actually sees. Nothing is broken: `TOLERANCE` is 1e-9, seven orders clear, and `test_the_tolerance_sits_between_the_noise_and_the_band_margin` still holds. But anyone tightening `TOLERANCE` should use 1.11e-16, not 5.6e-17, as the floor.
