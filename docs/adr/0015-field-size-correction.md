...
## Amended 2026-08-16: Rule B's standings guard is gone

The `max_placement < event_size` guard read a recorded last place as corroborating a small claimed field. The 2026-08-15 snapshot produced the counterexample: FSNS claims a field of 3 and records a 2nd and a 3rd, so the guard accepted 3 as a real field and scored that 3rd place 1.000. Per the repo owner, FSNS was an event of at least the floor where only a very small partial result was reported, the same shape as the other Rule B events. A podium-only upload will often include the deepest place it reports, so a recorded "last" is not evidence about a claimed field of 8 or less, and the domain rule (7PH runs no 8-player events) now decides every such Tournament unconditionally: `max(counted pilots, MIN_CUT_FIELD)`, recorded as `B`.

This also retires the one degenerate case the guard let stand: a self-consistent claimed field of 1 no longer passes through uncorrected, so no accepted field can ever be small enough to break the norm division.
