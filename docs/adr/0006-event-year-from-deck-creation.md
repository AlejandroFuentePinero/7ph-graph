...
There is no event date in the source. `events.json` carries only `lastUpdated`, which is when the record was touched, not when the event happened. The only usable signal is deck `createdAt` (present on all 4553 decks the raw snapshot carries), so an event's year is `min(createdAt)` across its decks.
...
- **Stated over the raw population, and now enforced over it.** Both counts above are raw-snapshot counts: `20260713T232944Z` holds 4553 decks across 107 events, and removing duplicate registrations (ADR 0004) takes it to 4552 decks across the same 107. That is a second population gap, of the same kind as the union one above and previously unnamed here: until issue #103 the build de-duplicated first and derived years second, so the guard was enforced over a population one deck smaller than the population this evidence is stated over, while the rule choosing which duplicate to drop never consults a date (it tie-breaks on placement, then deck id). The derivation now runs before the drop, so the guard reads the same decks the evidence does. It costs nothing on the data held: pre-drop and post-drop return identical years for all 108 events of the current two-snapshot union, and reading the larger population cannot lose an event key, since the survivor rule always keeps one member of every group.
