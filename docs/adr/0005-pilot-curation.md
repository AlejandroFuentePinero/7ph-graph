...
An absent file is not an error: the heuristics alone still build a graph. A malformed file, or one that contradicts itself, aborts the build. Four shapes contradict: a merged group naming two canonical ids, a `name` pin on an id that merges away, a pair carrying both a `merge` and a `reject` or `split`, and two `name` pins on one id (the last two added by issue #204, which found four such entries on file). The remedy the second one names is an edit, not an appended inverse: `merge` only ever exists for ids whose recovered names differ, which is the complement of what the identical-name join and its `split` and `reject` separators decide, so deleting a wrong `merge` is always sufficient and no precedence between the kinds is needed.
