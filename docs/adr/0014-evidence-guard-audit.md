...
| `placementNorm` | copied verbatim; divisor is the source's `eventSize` | "the tournament size the score is normalised against" | loudly, if at all | **clean at the time of the audit**: 4591 of 4591 matched the source; the duplicate drop moved 0. Superseded by issue #140, which found the divisor itself wrong at 9 of 108 events (an `eventSize` a count contradicts) and re-ranks 54 decks against a corrected field. The value is no longer verbatim at those 9, and the label above is now true of the whole set rather than of the source's number |
