...
## Amendment: a seventh tool, `best_player_race`, and its own ADR (issue #135)

The agent surface is seven functions. `best_player_race()` takes no argument, like `meta_share_over_time`, and returns the whole field of contenders as `RaceCell`s: one cell per contender per sample date, carrying both the pilot's career standing and what their record said at that date.
